...
from EE_read_versions import load_versions, load_versions_EE_Ensemble_OOV
...
def inter_map(average, target_filename, target_dim=950):
    lbl, word2vec, Huang, glove, collobert, target_version, overall_vocab, overlap_vocab=load_versions_EE_Ensemble_OOV(target_filename, target_dim)
    ...
    for j in range(len(version_list)):
        ...
        network=MAPPING(learning_rate=0.1, n_epochs=400000, batch_size=200, useAllSamples=1, 
                        L2_weight=0.00000000005, vali_cost_list_length=10, source_embedding_size=version_list[j].dim, 
                        target_embedding_size=target_dim, version1=target_version, version2=version_list[j])
    ...
    i_overall_OOV=overall_vocab-target_version.vocab
    OOV_file=open('/mounts/data/proj/wenpeng/Emb_Extend/results_L2norm_forall/'+target_filename[target_filename.rindex('/')+1:target_filename.rindex('.txt')]+'_OOV_embs_new.txt', 'w')
    if average:
        for word in i_overall_OOV:
            ...
            OOV_file.write('\n')
        OOV_file.close()
        print 'stored all OOV embs ', target_filename
...
if __name__ == '__main__':
    ...
    #inter_map(average=True, target_filename='/mounts/data/proj/wenpeng/Dataset/CW_L2norm_per_row.txt', target_dim=200)
    #inter_map(average=True, target_filename='/mounts/data/proj/wenpeng/Emb_Extend/results_L2norm_forall/O2M_bias_d200_0_1_2_3.txt', target_dim=200)
    inter_map(average=True, target_filename='/mounts/data/proj/wenpeng/Emb_Extend/results_L2norm_forall/conc_hs_wp_0_1_2_3.txt', target_dim=750)
